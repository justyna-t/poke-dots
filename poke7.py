...
from pygame import KEYUP, K_RETURN, QUIT, MOUSEBUTTONUP, Color
...
class Dot:
    ...
    def randomize_color(self):
        # Change the dot's color.
        # - self is a Dot
        colors = ["white", "yellow", "red", "blue", "purple", "green", "pink"]
        prev_color = self._color
        self._color = colors[randint(0, len(colors) - 1)]
        while self._color == prev_color:
            self._color = colors[randint(0, len(colors) - 1)]

    def randomize_second_dot_color(self, dot):
        # Change the other dot's color until it is different then
        # first dot's color.
        # - self is a Dot
        # - dot is the other Dot
        dot.randomize_color()
        while dot._color == self._color:
            dot.randomize_color()
